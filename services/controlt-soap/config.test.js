...
beforeEach(() => {
  _resetConfig();
  delete process.env.CONTROLT_USER;
  delete process.env.CONTROLT_PASS;
...
test('retorna config cuando todas las vars requeridas están presentes', () => {
  process.env.CONTROLT_USER = 'usuario_test';
  process.env.CONTROLT_PASS = 'clave_test';
  ...
  assert.equal(cfg.endpoint, 'https://app.controlt.com.co/WS/service.asmx');
  assert.equal(cfg.namespace, 'http://controlt.com.co/');
});

test('acepta CONTROLT_SOAP_TIMEOUT_MS personalizado', () => {
  process.env.CONTROLT_USER = 'u';
  process.env.CONTROLT_PASS = 'p';
...
test('lanza error si CONTROLT_USER falta', () => {
  process.env.CONTROLT_PASS = 'p';

  assert.throws(
    () => getConfig(),
    (err) => {
      assert.ok(err.message.includes('CONTROLT_USER'));
      return true;
    }
  );
});

test('lanza error si CONTROLT_PASS falta', () => {
  process.env.CONTROLT_USER = 'u';

  assert.throws(
    () => getConfig(),
    (err) => {
      assert.ok(err.message.includes('CONTROLT_PASS'));
      return true;
    }
  );
});

test('lanza error si CONTROLT_USER está vacía', () => {
  process.env.CONTROLT_USER = '';
  process.env.CONTROLT_PASS = 'p';
