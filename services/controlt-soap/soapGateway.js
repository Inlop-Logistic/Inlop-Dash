...
async function sendSoap(endpoint, soapAction, body, timeoutMs) {
  ...
  logSoapPayload(soapAction, 'response', xml);
  return xml;
}
...
export function makeLoginFn(config) {
  return async function loginFn() {
    ...
    // El cuerpo del Login contiene credenciales — no se registra el XML completo
    // en ningún nivel para evitar que aparezcan en logs. Solo se registra la respuesta.
    logSoapPayload('Login', 'request', `[Login body omitido — contiene credenciales | ${Buffer.byteLength(envelope, 'utf8')} bytes]`);
